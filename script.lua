...
local function ACBYPASS()
    if ACBYPASS_HASRUN then
        return
    end
    if not ACBYPASS_HASRUN then
        ACBYPASS_HASRUN = true
        local checkfunc = is_synapse_function or is_protosmasher_closure or function()
                return false
            end
        ...
        for i, v in next, gc do
            if type(v) == "function" and islclosure(v) and not checkfunc(v) then
                for k, x in next, getconstants(v) do
                    if
                        x and tonumber(x) and
                            ((tonumber(x) > 19 and tonumber(x) < 21) or (tonumber(x) > 49 and tonumber(x) < 51))
                     then
                        setconstant(v, k, 100)
                    end
                end
...
coroutine.resume(
    coroutine.create(
        function()
            repeat
                wait(.25)
            until EZSTATS_READY and ezstats and ezstats.walkspeed_enabled
            while wait(.25) do
...
coroutine.resume(
    coroutine.create(
        function()
            repeat
                wait(.25)
            until EZSTATS_READY and ezstats and ezstats.jumppower_enabled
            while wait(.25) do
...
coroutine.resume(
    coroutine.create(
        function()
            repeat
                wait(.25)
            until EZSTATS_READY and ezstats and ezstats.tacklereach_enabled
            while rstep:wait() do
...
coroutine.resume(
    coroutine.create(
        function()
            repeat
                wait(.25)
            until EZSTATS_READY and ezstats and ezstats.spamdab
...
coroutine.resume(
    coroutine.create(
        function()
            repeat
                wait(.25)
            until EZSTATS_READY and ezstats and ezstats.autoTacklePersuitEnabled
            while step:wait() do
...
coroutine.resume(
    coroutine.create(
        function()
            repeat
                wait(.25)
            until EZSTATS_READY and ezstats and ezstats.nobounds_enabled
            while wait() do
                local s, e =
                    pcall(
                    function()
...
coroutine.resume(
    coroutine.create(
        function()
            repeat
                wait(.25)
            until EZSTATS_READY and ezstats and ezstats.illegalForwardPass_enabled
...
coroutine.resume(
    coroutine.create(
        function()
            repeat
                wait(.25)
            until EZSTATS_READY and ezstats and ezstats.autoblock_enabled
                    ...
                    if
                     ...
                     then
                        ...
                        pos1 = ((pos2_dist <= 7 and pos2) or hrp2.Position)
                            ...
                            --[[ for i=1,10 do ]]
                            local cf1 =
                                CFrame.new(pos1, v3.new(lastpos.X, hrp.Position.Y, lastpos.Z) + v3.new(-lv.X, 0, -lv.Z))
                            hrp.CFrame = cf1
...
coroutine.resume(
    coroutine.create(
        function()
            repeat
                wait(.25)
            until EZSTATS_READY and ezstats and ezstats.blockRangeEnabled
            char:WaitForChild("BlockPart")
            local function modifyBlockRange(range)
                if char and hum and hrp then
                    local blockPart = char:WaitForChild("BlockPart")
...
coroutine.resume(
    coroutine.create(
        function()
            repeat
                wait(.25)
            until EZSTATS_READY and ezstats and ezstats.QbAimbotEnabled
                    ...
                    fbtool.Equipped:connect(
                        function(p4)
                            ...
                            if Head14 and not throwAnim.IsPlaying and not hum.PlatformStand then
                                ...
                                FireServer1(
                                    fbtool:WaitForChild("Handle"):WaitForChild("RemoteEvent"),
                                    "Clicked",
                                    Head14.CFrame.p,
                                    pos,
                                    power
                    ...
                    if v.Name ~= plr.Name then
                        if
                         ...
                         then
                            ...
                            local check =
                                (Vector2.new(mouse.X, mouse.Y) - Vector2.new(screenpoint.X, screenpoint.Y)).magnitude
                            if check < dist then
                                target = v
                                dist = check
                            end
...
coroutine.resume(
    coroutine.create(
        function()
            repeat
                wait(.25)
            until EZSTATS_READY and ezstats and ezstats.kickerAimbotEnabled
            ...
            local function kick()
                if ezstats.kickerAimbotEnabled then
                    ...
                    until cursor.Position.Y.Scale < 0.01
                     --cursor.Position.Y.Scale == math.clamp(cursor.Position.Y.Scale,.001,.005)
...
coroutine.resume(
    coroutine.create(
        function()
            repeat
                wait(.25)
            until EZSTATS_READY and ezstats and ezstats.AutoHands
            while rstep:wait() do
                pcall(
                    function()
                        ...
                        if fb then
                            if
                                values.Status.Value == "InPlay" and values.QB.Value ~= plr and
                        ...
                        elseif
                            ezstats.AutoHands and values.Carrier.Value and values.QB.Value and
...
coroutine.resume(
    coroutine.create(
        function()
            repeat
                wait(.25)
            until EZSTATS_READY and ezstats and ezstats.mags_enabled
            local Settings = {
            ...
            local function mag()
                if qb.Value == plr then
                    return resize_catchpart(0)
                end
                local magging = true
                if not executedCount == getgenv().executed_count then
                    return resize_catchpart(0)
                end
                local football = workspace:FindFirstChild("Football")
                if not football or values.Status.Value ~= "InPlay" then
                    return resize_catchpart(0)
                end
                ...
                repeat
                    step:wait()
                until not football or football.Parent ~= workspace
                pcall(
                    function()
                        resize_catchpart(50)
                    end
...
coroutine.resume(
    coroutine.create(
        function()
            repeat
                wait(.25)
            until EZSTATS_READY and ezstats and ezstats.fly_enabled
...
coroutine.resume(
    coroutine.create(
        function()
            repeat
                wait(.25)
            until EZSTATS_READY and ezstats and ezstats.antiJam_enabled
                        ...
                        function()
                            ...
                            if isBasePart then
                                ...
                                while step:wait() and v:IsA("BasePart") do
                                    ...
                                    if
                                        enabled and ws:FindFirstChild("Football") and status.Value == "InPlay" and
                                            playtype.Value ~= "fieldgoal"
                                     then
                                        v.CanCollide = false
                                    else
...
coroutine.resume(
    coroutine.create(
        function()
            repeat
                wait(.25)
            until EZSTATS_READY and ezstats and ezstats.followcarrier_enabled
                            ...
                            if ezstats.followcarrier_randomFactorMin == 0 or ezstats.followcarrier_randomFactorMax == 0 then
                                ...
                                local s, e =
                                    ...
                                    function()
                                        randomFactor =
                                            math.random(
                                            -ezstats.followcarrier_randomFactorMin,
                                            ezstats.followcarrier_randomFactorMax
                                        )
                                    end
                                )
                                if not s then
            ...
            local function getTacklePos()
                ...
                if plr2 and status.Value == "InPlay" then
                    ...
                    if char2 then
                        ...
                        local pos =
                            getPosInXSeconds(hrp2.Position, hum2.MoveDirection, hum2.WalkSpeed + randomFactor, time)
                        return pos
                    end
                end
            end
            while wait(ezstats.followcarrier_posUpdateDelay) do
                ...
                if ezstats.followcarrier_enabled then
                    if
                        carrier.Value and carrier.Value.Team ~= plr.Team and carrier.Value ~= plr and
                            status.Value == "InPlay" and
                            not isQB
                     then
...
coroutine.resume(
    coroutine.create(
        function()
            repeat
                wait(.25)
            until EZSTATS_READY and ezstats and ezstats.divePowerEnabled
            ...
            uis.InputBegan:Connect(
                function(k, gpe)
                    if
                        ezstats.divePowerEnabled and not gpe and k.KeyCode == Enum.KeyCode.E and not debounce and
                     ...
                     then
                        ...
                        repeat
                            step:wait()
                        until hrp.Position.Y <= 3.5 or tick() - t >= 3
                        bf:Destroy()
                        debounce = false
                    end
                end
...
coroutine.resume(
    coroutine.create(
        function()
            repeat
                wait(.25)
            until EZSTATS_READY and ezstats and ezstats.ballTP_enabled
...
coroutine.resume(
    coroutine.create(
        function()
            repeat
                wait(.25)
            until EZSTATS_READY and ezstats and ezstats.autocap
...
coroutine.resume(
    coroutine.create(
        function()
            repeat
                wait(.25)
            until EZSTATS_READY and ezstats and ezstats.instaScoreEnabled
...
coroutine.resume(
    coroutine.create(
        function()
            repeat
                wait(.25)
            until EZSTATS_READY and ezstats and ezstats.autoJumpEnabled
            while rstep:wait() do
...
coroutine.resume(
    coroutine.create(
        function()
            repeat
                wait(.25)
            until EZSTATS_READY and ezstats and ezstats.antiLagEnabled
